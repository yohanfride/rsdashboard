...
class MY_Model extends CI_Model {
	...
	function sendPost($url,$data,$session=""){
        $auth = "";
        if($session){
            $auth = "Authorization: Basic $session\r\n";
        }
        $options = array(
                'http' => array(
    ...
    function sendPostJson($url,$data,$session=""){      
        $auth = "";
        if($session){
            $auth = "Authorization: Basic $session\r\n";
        }
    ...
    function sendCurlJson($url,$data){
    ...
    function page($jml='', $perhalaman='' ,$hal=''){
        ...
        $output = '<nav class="d-inline-block">
        ...
        if ($nohalaman > 1){
            ...
            $output .= '<li class="page-item"><a class="page-link" href="?'.$query.'" tabindex="0">Previous</a></li>';
        } else {        
            $output .= '<li class="page-item disabled"><a class="page-link" href="#" tabindex="0">Previous</a></li>';
        }
            ...
            $query = http_build_query($params);
                 ...
                 {   
                    if (($showhalaman == 1) && ($halaman != 2)){  
                        ...
                        $output .= '<li class="page-item active"><a class="page-link" href="#">'.$halaman.' <span class="sr-only">(current)</span></a></li>';
                    }else{ 
                        $output .= '<li class="page-item"><a class="page-link" href="?'.$query.'">'.$halaman.'</a></li>';
                    }
                    $showhalaman = $halaman;          
                 }
        }
        ...
        if ($nohalaman < $jumhalaman){ 
            ...
            $query = http_build_query($params);
            $output .= '<li class="page-item"><a class="page-link" href="?'.$query.'">Next</a></li>';
        } else {
