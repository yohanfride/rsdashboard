...
class Room_m extends My_Model{
	...
	function get_detail($id,$token){
		...
		$url = $this->config->item('url_node')."ruangan/get/";				
		return json_decode($this->sendPostJson($url,$data,$token));
	}

	function add($data,$token){
		$url = $this->config->item('url_node')."ruangan/insert/";				
		return json_decode($this->sendPostJson($url,$data,$token));
	}

	function edit($id,$data,$token){
		$data+=["id" => $id];
		$url = $this->config->item('url_node')."ruangan/update/";				
		return json_decode($this->sendPostJson($url,$data,$token));
	}
	...
	function del($id,$token){
		...
		$url = $this->config->item('url_node')."ruangan/delete/";				
		return json_decode($this->sendPostJson($url,$data,$token));
	}	
	...
	function search_count($data,$token){
		$url = $this->config->item('url_node')."ruangan/total/";				
		return json_decode($this->sendPostJson($url,$data,$token));
	}
